...
const SponsorGame: React.FC = () => {
      ...
      try {
        ...
        const dropDetailsPromises = unfundedDropIds.map(async (dropId) => {
          ...
          if (
            ...
            if (isAddress(host)) hostAddressesToFetch.add(getAddress(host));
  ...
  const handleFundDrop = async (dropToFund: SponsoredDropToFund) => {
    ...
    try {
      ...
      // Parse dropId from GameFunded event
      let parsedDropId: string = dropToFund.dropId.toString(); // Fallback
      try {
        for (const log of receipt.logs) {
          ...
          try {
            ...
            if (
              decodedEvent.eventName === "GameFunded" &&
              decodedEvent.args &&
              typeof (decodedEvent.args as any).dropId === "bigint"
            ) {
              parsedDropId = (decodedEvent.args as any).dropId.toString();
              console.log(`[FundDrop] Parsed dropId: ${parsedDropId}`);
              break;
            }
          } catch (e) {
            console.warn(`[FundDrop] Could not decode log: ${e}`);
          }
        }
        if (parsedDropId === dropToFund.dropId.toString()) {
          console.log("[FundDrop] GameFunded event confirmed dropId");
        } else {
          console.warn(
            `[FundDrop] Mismatch in dropId: expected ${dropToFund.dropId}, got ${parsedDropId}`
          );
        }
      } catch (e) {
        console.error(`[FundDrop] Error parsing dropId: ${e}`);
      }

      // Send game-sponsored notification by calling the API endpoint
      try {
        const notificationPayload = {
          category: "game-sponsored",
          hostAddress: dropToFund.host,
          sponsorAddress: address,
          dropId: parsedDropId, // Use parsed dropId
          rewardType: dropToFund.rewardType,
          rewardAmount: dropToFund.rewardAmountDisplay,
          maxParticipants: Number(dropToFund.maxParticipants),
          numWinners: dropToFund.numWinners,
          transactionHash: fundTxHash,
        };
        ...
        if (!response.ok) {
          ...
          console.error(
            "Failed to send game-sponsored notification:",
            errorData.message
          );
        ...
        console.error(
          "Error calling send-notification API for game-sponsored:",
          notificationError
        );
        toast.warn("Failed to send notification");
      }

      toast.success(`Drop #${parsedDropId} funded successfully!`);
