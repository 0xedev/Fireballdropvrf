  ...
  useEffect(() => {
    const fetchDropsAndHostProfiles = async () => {
      ...
      try {
        ...
        // Sort drops by ID in descending order to show recent ones first?
        const sortedDrops = resolvedDrops.sort((a, b) => b.id - a.id);
        setDrops(sortedDrops);

        // After fetching drops, fetch profiles for all unique host and sponsor addresses//come back
